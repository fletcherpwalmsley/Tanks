/*===========================================
          ...
          Builds on box_bounce.
          Adds the abilty to click to
                spawn a box
...
b2BodyId MakeRotatingBox(float width, float height) {
    b2BodyDef bodyDef = b2DefaultBodyDef();
    bodyDef.type = b2_dynamicBody;
    bodyDef.position = (b2Vec2){width, height};
    bodyDef.angularVelocity = -3.14f;

    b2BodyId bodyId = b2CreateBody(worldId, &bodyDef);
    b2Polygon dynamicBox = b2MakeBox(BoxLengthMetres/2.0, BoxLengthMetres/2.0);
...
int main(int argc, char **argv) {
    ...
    while(1) {
        ...
        if (WPADKeyDown & WPAD_BUTTON_B) {
            boxInfo box;
            box.body = MakeRotatingBox((P1MX) /WorldToWindowScale, (480 - P1MY)/WorldToWindowScale);
