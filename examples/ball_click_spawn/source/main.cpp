...
u_int32_t& RandomColour() {
  ...
  static std::vector<u_int32_t> colours = {0x000000FF, 0x800000FF, 0x008000FF, 0x808000FF, 0x000080FF, 0x800080FF, 0x008080FF, 0x808080FF,
                                           0xC0C0C0FF, 0xFF0000FF, 0x00FF00FF, 0xFFFF00FF, 0x0000FFFF, 0xFF00FFFF, 0x00FFFFFF};
...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    for (auto box : boxes) {
      ...
      GRRLIB_Circle(position.x * WorldToWindowScale, 480 - position.y * WorldToWindowScale, 2.5 * WorldToWindowScale, box.colour, 1);
      GRRLIB_Line(position.x * WorldToWindowScale, 480 - position.y * WorldToWindowScale, (position.x * WorldToWindowScale) + (directionUnitVector.x * 40),
